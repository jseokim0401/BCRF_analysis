...
# In Seurat v5, when a merged object keeps its layers split by dataset, 
# running NormalizeData() → FindVariableFeatures() → ScaleData() → RunPCA()
# on the merged object computes each of those steps per layer/per dataset automatically 
...
setwd("/Users/eguo/Downloads/integrated BCRF set 1-10 & EN/integrate BCRF v2")
...
log2counts <- FeaturePlot(merged_obj, features = "log2counts", combine = TRUE)
print(log2counts)
ggsave("log2counts.png", width = 8, height = 8)

FeaturePlot(merged_obj, features = c("ESR1", "ERBB2", "MYC", "TP53", "PIK3CA"), cols = c("red", "blue"))
ggsave("genes.png", width = 8, height = 8)

saveRDS(merged_obj, file = "/Users/eguo/Downloads/integrated BCRF set 1-10 & EN/integrated_BCRF_set_1-10_&_EN_v2.rds")
